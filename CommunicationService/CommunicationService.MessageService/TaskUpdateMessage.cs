...
namespace CommunicationService.MessageService
{
    public class TaskUpdateMessage : IMessage
    {
        ...
        public async Task<EmailBuildData> PrepareTemplateData(Guid batchId, int? recipientUserId, int? jobId, int? groupId, string templateName)
        {
            ...
            jobStatusRequest.JobStatuses = new List<JobStatuses>() { job.JobSummary.JobStatus };
            jobRequest.Postcode = job.JobSummary.PostCode;
            ...
            {
                HelpMyStreet.Utils.Models.JobSummary summary = jobsResponse.JobSummaries.FirstOrDefault(x => x.JobID == job.JobSummary.JobID);
                int groupID_ageuk = -3;
