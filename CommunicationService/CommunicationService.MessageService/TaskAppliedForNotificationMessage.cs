...
{
    public class TaskAppliedForNotificationMessage : IMessage
    {
        ...
        public TaskAppliedForNotificationMessage(IConnectRequestService connectRequestService, IConnectGroupService connectGroupService, IConnectUserService connectUserService, ILinkRepository linkRepository, IOptions<LinkConfig> linkConfig)
        {
        ...
        {
            return UnsubscribeGroupName.TaskAppliedForNotification;
        }
        ...
        public async Task<List<SendMessageRequest>> IdentifyRecipients(int? recipientUserId, int? jobId, int? groupId, int? requestId, Dictionary<string, string> additionalParameters)
        {
            ...
            foreach(int userId in groupTaskAdmins)
            {
                ...
                {
                    TemplateName = TemplateName.TaskAppliedForNotification,
        ...
        public async Task<EmailBuildData> PrepareTemplateData(Guid batchId, int? recipientUserId, int? jobId, int? groupId, int? requestId, Dictionary<string, string> additionalParameters, string templateName)
        {
            ...
            if (requestDetails != null && user?.UserPersonalDetails != null && group?.Group != null)
            {
                ...
                {
                    BaseDynamicData = new TaskAppliedForNotificationData
                    (
                        title: "A request requires you to approve before the volunteer can proceed",
                        subject: "A request requires you to approve before the volunteer can proceed",
